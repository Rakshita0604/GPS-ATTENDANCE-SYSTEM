...
export function RecordsReports() {
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((record) =>
      [record.name, record.email, record.employee_code, record.attendance_date, record.status]
        .filter(Boolean)
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-900">Records & Reports</h2>
        <p className="text-slate-500">Company-wide attendance records.</p>
      </div>
      ...
      <Card>
        <CardHeader className="border-b border-slate-100 bg-slate-50/50">
          <div className="relative w-full sm:w-96">
            ...
            <Input
              type="search"
              placeholder="Search employee, email, status..."
                ...
                <TableRow key={record.id}>
                  <TableCell>
                    <div className="flex flex-col">
                      <span className="font-semibold text-slate-900">{record.name || "Employee"}</span>
                      <span className="text-xs text-slate-500">{record.employee_code || record.email}</span>
                    </div>
                  </TableCell>
